...
_find_cached_python() {
    local bin
    bin=$(find "$PBS_DIR/bin" -name "python3*" -maxdepth 1 -type f -executable 2>/dev/null | sort -V | tail -1 || true)
    if [ -z "$bin" ]; then
        bin=$(find "$PBS_DIR" -name "python3" -type f -executable 2>/dev/null | head -1 || true)
    fi
    echo "$bin"
}
